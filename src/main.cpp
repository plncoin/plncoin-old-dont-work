...
uint256 hashGenesisBlock("0x58c63157cc65022e582ebf3b192fa43b9ba8558eeb26954706d8d3c6924853e5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PLNcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PLNcoin Signed Message:\n";
...
{
    // PLNcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
//2mln premining
if (nHeight==1) return (2000000 * COIN) + nFees;

    // Subsidy is cut in half every 438000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 438000); // PLNcoin: 438k blocks in ~ 2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 60 * 60; // PLNcoin: 4 hours
static const int64 nTargetSpacing = 1 * 60; // PLNcoin: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PLNcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
        ...
        if (fTestNet) {
                ...
                if (pindexLast->nHeight+1 >= 2346) { DiffMode = 2; }
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PLNcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PLNcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x54790e8cdee8c1c9d2e9aeeab70b5e8fb0efc88b8dd96ba194634eb8f49ba91f");
    }
...
bool InitBlockIndex() {
	...
	//block.nNonce = 0;        
        ...
        assert(block.hashMerkleRoot == uint256("0xf6513239070801245a6707a88beea6dfaf818cf6e48c4375c3b9ef2f31f2235f"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // PLNcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PLNcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PLNcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PLNcoinMiner(CWallet *pwallet)
{
    printf("PLNcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("plncoin-miner");
    ...
    try { while(true) {
        ...
        printf("Running PLNcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PLNcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PLNcoinMiner, pwallet));
}
